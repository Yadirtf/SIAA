...
void main() {
  group('US-GEO-03: Captura por toque sobre mapa satelital', () {
    test(
        'AC-01: Al tocar sobre el mapa se añade un vértice en [longitud, latitud]',
        () async {
      final bloc = GeoEditorBloc();

      bloc.add(const CambiarModoCapturaRequested(ModoCapturaEditor.mapa));
      await bloc.stream
          .firstWhere((s) => s.modoCaptura == ModoCapturaEditor.mapa);
    ...
    test(
        'AC-02: Polígono dibujado exclusivamente por toque registra TOQUE_MAPA y precisionPromedioCalculada == null',
        () async {
    ...
    test(
        'AC-03: Alternar entre recorrido y mapa conserva vértices y registra MIXTO al combinar ambos',
        () async {
      String? metodoGuardado;
      double? precisionGuardada;
      ...
      bloc.add(const CambiarModoCapturaRequested(ModoCapturaEditor.mapa));
      final modoMapaState = await bloc.stream
          .firstWhere((s) => s.modoCaptura == ModoCapturaEditor.mapa);
    ...
    test(
        'AC-05: Deshacer retira vértice tanto de vertices como de verticesEtiquetados',
        () async {
      final bloc = GeoEditorBloc();
