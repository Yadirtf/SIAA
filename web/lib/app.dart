    ...
    return MultiRepositoryProvider(
      providers: [
        RepositoryProvider<AuthRepository>(
          create: (_) => AuthRepositoryImpl(remoteDataSource: AuthRemoteDataSource()),
        ),
        RepositoryProvider<GeoRepository>(
          create: (_) => GeoRepositoryImpl(remoteDataSource: GeoRemoteDataSource()),
        ),
        RepositoryProvider<AcademicoRepository>(
          create: (_) => AcademicoRepositoryImpl(remoteDataSource: AcademicoRemoteDataSource()),
        ),
      ],
      child: MultiBlocProvider(
        providers: [
          ...
          BlocProvider<GeoBloc>(
            create: (ctx) => GeoBloc(
              repository: ctx.read<GeoRepository>(),
            ),
          ),
          BlocProvider<AcademicoBloc>(
            create: (ctx) => AcademicoBloc(
              repository: ctx.read<AcademicoRepository>(),
            ),
          ),
        ],
        child: MaterialApp(
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AuthBloc, AuthState>(
      buildWhen: (previous, current) =>
          previous.runtimeType != current.runtimeType || current is Authenticated || current is Unauthenticated,
