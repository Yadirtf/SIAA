...
class _EstructuraScreenState extends State<EstructuraScreen> with SingleTickerProviderStateMixin {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            TextField(controller: codigoCtrl, decoration: const InputDecoration(labelText: 'Código (ej: FAC-ING)')),
            const SizedBox(height: 12),
            TextField(controller: nombreCtrl, decoration: const InputDecoration(labelText: 'Nombre de la Facultad')),
          ],
        ),
        actions: [
            ...
            onPressed: () {
              if (codigoCtrl.text.isNotEmpty && nombreCtrl.text.isNotEmpty) {
                context.read<AcademicoBloc>().add(
                      CreateFacultadEvent(codigo: codigoCtrl.text.trim(), nombre: nombreCtrl.text.trim()),
                    );
                Navigator.pop(ctx);
              }
            },
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (c, setMState) => AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: codigoCtrl, decoration: const InputDecoration(labelText: 'Código (ej: PROG-SIS)')),
              const SizedBox(height: 12),
              TextField(controller: nombreCtrl, decoration: const InputDecoration(labelText: 'Nombre del Programa')),
              const SizedBox(height: 12),
              DropdownButtonFormField<String>(
                value: selectedFacId,
                decoration: const InputDecoration(labelText: 'Facultad'),
                items: state.facultades.map((f) => DropdownMenuItem(value: f.id, child: Text(f.nombre))).toList(),
                onChanged: (val) {
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('Jerarquía académica institucional: Facultades, Programas y Cursos', style: AppTextStyles.bodyMedium),
                        ],
                      ),
                    ),
                    ElevatedButton.icon(
  ...
  Widget _buildFacultadesList(AcademicoLoaded state) {
    if (state.facultades.isEmpty) return const Center(child: Text('No hay facultades registradas.'));
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        return Card(
          child: ListTile(
            leading: const Icon(Icons.account_balance_outlined, color: AppColors.primaryAccent),
            title: Text('${f.nombre} (${f.codigo})', style: AppTextStyles.h3),
            trailing: IconButton(
              icon: const Icon(Icons.delete_outline, color: AppColors.accentRose),
              onPressed: () => context.read<AcademicoBloc>().add(DeleteFacultadEvent(f.id)),
            ),
          ),
        );
      },
  ...
  Widget _buildProgramasList(AcademicoLoaded state) {
    if (state.programas.isEmpty) return const Center(child: Text('No hay programas registrados.'));
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        return Card(
          child: ListTile(
            ...
            trailing: IconButton(
              icon: const Icon(Icons.delete_outline, color: AppColors.accentRose),
              onPressed: () => context.read<AcademicoBloc>().add(DeleteProgramaEvent(p.id)),
            ),
          ),
        );
      },
  ...
  Widget _buildAsignaturasList(AcademicoLoaded state) {
    if (state.asignaturas.isEmpty) return const Center(child: Text('No hay asignaturas registradas.'));
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        return Card(
          child: ListTile(
            ...
            subtitle: Text('Créditos: ${a.creditos}', style: AppTextStyles.bodyMedium),
          ),
        );
      },
    );
  ...
  Widget _buildGruposList(AcademicoLoaded state) {
    if (state.grupos.isEmpty) return const Center(child: Text('No hay grupos creados.'));
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        return Card(
          child: ListTile(
            ...
            subtitle: Text('Cupo: ${g.cupo} estudiantes', style: AppTextStyles.bodyMedium),
          ),
        );
      },
    );
